...
public class VFS1 {
    ...
    public void format(long totalBytes) {
        ...
        // --- Write Header Fields ---
        // Magic Number
        batchFill(OFF_MAGIC, "ATLAS_VFS1".getBytes(StandardCharsets.US_ASCII));
    ...
    private boolean validateHeaderBasics() {
        ...
        try {
            ...
            if (!"ATLAS_VFS1".equals(magic)) {
                 System.err.println("Header Check Failed: Magic number mismatch. Expected 'ATLAS_VFS1', got '" + magic + "'");
                return false;
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("--- ATLAS VFS1 Console ---");
                ...
                switch (command.toLowerCase()) {
                    ...
                    case "format": {
                        ...
                        try {
                            ...
                            if (bytes > 0) {
                                vfs = new VFS1(); // Create a new instance to clear old state
                                vfs.format(bytes);
                    ...
                    case "load": {
                         ...
                         try {
                             vfs = new VFS1(); // Create new instance before loading
                             vfs.loadDisk(line);
                             currentVfsFile = line; // Store path on successful load
                         } catch (RuntimeException e) {
                             System.err.println("Load Error: " + e.getMessage());
                             vfs = new VFS1(); // Ensure vfs is reset to empty state on error
                             currentVfsFile = null;
                         }
                         break;
                    }
                    ...
                    case "list": {
                        try {
                            ...
                            for (int i = 0; i < maxFiles; i++) {
                                long entryOffset = (long)headerEndAddress + ((long)i * VFS1.FILE_ENTRY_SIZE);
                                if (vfs.readAt(entryOffset + VFS1.FENTRY_OFF_EXISTS) == 1) {
                                    String name = vfs.readString(entryOffset + VFS1.FENTRY_OFF_FILENAME, 64);
                                    int len = vfs.readInt(entryOffset + VFS1.FENTRY_OFF_SIZE_BYTES);
                                    ...
                                    String modDate = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date(modifiedMs));
